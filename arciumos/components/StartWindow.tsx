...
export default function StartWindow({
	...
	return (
		<Draggable nodeRef={nodeRef} handle=".drag-handle" cancel="button">
			...
			>
				<div className="flex drag-handle flex-row justify-between bg-stone-300 h-8 w-full items-center px-4 rounded-t-md cursor-move select-none" style={{ touchAction: 'none' }}>
					<span className="flex items-center gap-1"><FileIcon /> <p>Welcome.txt</p></span>
					<Button variant={"ghost"} onClick={() => onClose?.()}><CircleXFillIcon/></Button>
				</div>
